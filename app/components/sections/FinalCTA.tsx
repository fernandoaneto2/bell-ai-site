import Image from "next/image";
import Container from "@/components/ui/Container";
import Button from "@/components/ui/Button";

export default function FinalCTA() {
  return (
    <section
      id="book-demo"
      className="relative py-24 md:py-32 bg-bg-primary overflow-hidden"
      aria-label="Book a demo"
    >
      {/* Radial gold glow */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 80% 80% at 50% 50%, rgba(201,162,76,0.07) 0%, transparent 70%)",
        }}
        aria-hidden="true"
      />

      {/* Top hairline */}
      <div
        className="absolute top-0 left-0 right-0 h-px"
        style={{
          background: "linear-gradient(90deg, transparent, rgba(201,162,76,0.4), transparent)",
        }}
        aria-hidden="true"
      />

      <Container className="relative z-10">
        <div className="max-w-2xl mx-auto text-center flex flex-col gap-6 items-center">

          <Image
            src="/Logo_BELL_sem_fundo.png"
            alt="bell.ai"
            width={120}
            height={120}
            className="h-28 w-28 object-contain drop-shadow-[0_0_28px_rgba(201,162,76,0.4)]"
          />

          {/* Urgency badge */}
          <div
            className="inline-flex items-center gap-2 bg-[rgba(201,162,76,0.08)] border border-[rgba(201,162,76,0.25)] rounded-full px-4 py-1.5"
            role="status"
            aria-live="polite"
          >
            <span
              className="w-2 h-2 rounded-full bg-gold-primary"
              style={{ animation: "pulse 2s cubic-bezier(0.4,0,0.6,1) infinite" }}
              aria-hidden="true"
            />
            <span className="text-xs text-gold-primary font-medium">
              Only 5 spots remaining for free onboarding this month
            </span>
          </div>

          <h2
            className="font-sans font-light text-white-pure text-balance leading-tight"
            style={{ fontSize: "clamp(2rem, 5vw, 3.5rem)" }}
          >
            Your guests are already asking.
            <br />
            <span className="font-semibold">Make sure someone&apos;s answering.</span>
          </h2>

          <p className="text-gray-muted text-lg max-w-lg">
            Book a free 20-minute demo. See bell.ai trained live on your establishment — no commitment required.
          </p>

          <div className="flex flex-col sm:flex-row gap-3 mt-2 w-full sm:w-auto justify-center">
            {/* Primary CTA — TODO: replace href with real Calendly/Cal.com link */}
            <Button
              variant="gold-solid"
              size="lg"
              href="#"
              className="min-w-52"
            >
              Book Free Demo →
            </Button>

            {/* Secondary CTA — WhatsApp */}
            {/* TODO: replace phone number with real WhatsApp business number */}
            <Button
              variant="ghost"
              size="lg"
              href="https://wa.me/5500000000000"
            >
              <svg
                viewBox="0 0 24 24"
                fill="currentColor"
                className="w-4 h-4 text-green-400"
                aria-hidden="true"
              >
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              Chat on WhatsApp
            </Button>
          </div>

          <p className="text-gray-muted text-xs opacity-50 mt-2">
            No credit card required · Setup in under 7 days · Cancel anytime
          </p>
        </div>
      </Container>

      {/* Bottom hairline */}
      <div
        className="absolute bottom-0 left-0 right-0 h-px"
        style={{
          background: "linear-gradient(90deg, transparent, rgba(201,162,76,0.2), transparent)",
        }}
        aria-hidden="true"
      />
    </section>
  );
}
